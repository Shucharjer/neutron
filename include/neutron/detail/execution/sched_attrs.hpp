...
public:
    ...
    constexpr explicit _sched_attrs(S&& sched)
        : sched_(::std::forward<S>(sched)) {}

    template <typename Tag>
    constexpr auto
        query(const get_completion_scheduler_t<Tag>&) const noexcept {
        return this->sched_;
    }

    template <typename T = bool>
    requires requires(Scheduler&& sched) { sched.query(get_domain); }
    constexpr auto query(const get_domain_t& qry, T = true) const noexcept {
        return this->sched_.query(qry);
    }
};
