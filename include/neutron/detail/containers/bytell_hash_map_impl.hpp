...
// IWYU pragma: private, include <neutron/bytell_hash_map.hpp>
...
struct sherwood_v8_block {
    sherwood_v8_block() noexcept {}
    ~sherwood_v8_block() noexcept {}
    int8_t control_bytes[BlockSize]; // NOLINT
    union {
        T data[BlockSize];
    };

    static sherwood_v8_block* empty_block() noexcept {
    ...
    ATOM_NODISCARD constexpr int first_empty_index() const noexcept {
    ...
    constexpr void fill_control_bytes(int8_t value) {
...
public:
    ...
    constexpr sherwood_v8_table() {}
    constexpr explicit sherwood_v8_table(
    ...
    sherwood_v8_table(const sherwood_v8_table& other)
        : sherwood_v8_table(
              other, AllocatorTraits::select_on_container_copy_construction(
                         other.get_allocator())) {}
    sherwood_v8_table(
        const sherwood_v8_table& other, const ArgumentAlloc& alloc)
    ...
    sherwood_v8_table(sherwood_v8_table&& other) noexcept
        : ByteAlloc(std::move(other)), Hasher(std::move(other)),
          Equal(std::move(other)), _max_load_factor(other._max_load_factor) {
        swap_pointers(other);
    }
    sherwood_v8_table(
        sherwood_v8_table&& other, const ArgumentAlloc& alloc) noexcept
    ...
    sherwood_v8_table& operator=(const sherwood_v8_table& other) {
        ...
        if (AllocatorTraits::propagate_on_container_copy_assignment::value) {
            if (static_cast<ByteAlloc&>(*this) !=
                static_cast<const ByteAlloc&>(other)) {
                reset_to_empty_state();
            }
            AssignIfTrue<
                ByteAlloc, AllocatorTraits::
    ...
    sherwood_v8_table& operator=(sherwood_v8_table&& other) noexcept {
        ...
        if (AllocatorTraits::propagate_on_container_move_assignment::value) {
            ...
            static_cast<ByteAlloc&>(*this) == static_cast<ByteAlloc&>(other)) {
            swap_pointers(other);
        } else {
            clear();
            _max_load_factor = other._max_load_factor;
    ...
    private:
        ...
        BlockPointer current_ = BlockPointer();
        size_t index_         = 0;

    public:
        templated_iterator() {}
        templated_iterator(BlockPointer entries, size_t index)
            : current_(entries), index_(index) {}
        ...
        templated_iterator& operator++() {
            do {
                if (index_ % BlockSize == 0)
                    --current_;
                if (index_-- == 0)
                    break;
            } while (current_->control_bytes[index_ % BlockSize] ==
                     Constants::magic_for_empty);
            return *this;
        }
        ...
        ValueType& operator*() const {
            return current_->data[index_ % BlockSize];
        }
        ValueType* operator->() const {
            return current_->data + (index_ % BlockSize);
        }
    ...
    iterator begin() {
        size_t num_slots =
            num_slots_minus_one != 0 ? num_slots_minus_one + 1 : 0;
    ...
    iterator find(const FindKey& key) noexcept {
        ...
        for (;;) {
            ...
            if (first) {
                if ((metadata & Constants::bits_for_direct_hit) !=
                    Constants::magic_for_direct_hit) {
                    return end();
                }
                first = false;
            }
            ...
            index += Constants::jump_distances[to_next_index]; // NOLINT
            index = hash_policy.keep_in_range(index, num_slots_minus_one);
        }
    }
    const_iterator find(const FindKey& key) const noexcept {
        return const_cast<sherwood_v8_table*>(this)->find(key);
    }
    size_t count(const FindKey& key) const noexcept {
        return find(key) == end() ? 0 : 1;
    }
    std::pair<iterator, iterator> equal_range(const FindKey& key) noexcept {
            ...
            if (first) {
                if ((metadata & Constants::bits_for_direct_hit) !=
                    ...
                    return emplace_direct_hit(
                        { index, block }, std::forward<Key>(key),
    ...
    void rehash(size_t num_items) {
        ...
        unsigned char* new_memory =
            &*AllocatorTraits::allocate(*this, memory_requirement);
        ...
        if (num_items % BlockSize)
            ++old_num_blocks;
        for (BlockPointer it = new_buckets, end = new_buckets + old_num_blocks;
             it != end; ++it) {
            for (int i = 0; i < BlockSize; ++i) {
                int8_t metadata = it->control_bytes[i];
                if (metadata != Constants::magic_for_empty &&
                    metadata != Constants::magic_for_reserved) {
    ...
    convertible_to_iterator erase(const_iterator to_erase) {
        LinkedListIt current = { to_erase.index_, to_erase.current_ };
        if (current.has_next()) {
            ...
            AllocatorTraits::construct(
                *this, std::addressof(*current), std::move(*next));
    ...
    iterator erase(const_iterator begin_it, const_iterator end_it) {
        ...
        std::sort(
            depth_in_chain.begin(), depth_in_chain.end(),
            [](const auto& a, const auto& b) { return a.first < b.first; });
        for (auto it = depth_in_chain.rbegin(), end = depth_in_chain.rend();
             it != end; ++it) {
            erase(it->second.it());
        }

        if (begin_it.current_->control_bytes[begin_it.index_ % BlockSize] ==
            Constants::magic_for_empty)
            return ++iterator{ begin_it.current_, begin_it.index_ };
        else
            return { begin_it.current_, begin_it.index_ };
    }
    ...
    void clear() {
        ...
        if (num_slots % BlockSize)
            ++num_blocks;
        for (BlockPointer it = entries, end = it + num_blocks; it != end;
             ++it) {
            for (int i = 0; i < BlockSize; ++i) {
                if (it->control_bytes[i] != Constants::magic_for_empty) {
                    AllocatorTraits::destroy(
                        *this, std::addressof(it->data[i]));
...
private:
    ...
    bool is_full() const {
        if (!num_slots_minus_one)
            ...
            return num_elements + 1 > (num_slots_minus_one + 1) *
    ...
    struct LinkedListIt {
        ...
        iterator it() const { return { block, index }; }
        int index_in_block() const { return index % BlockSize; }
        bool is_direct_hit() const {
            return (metadata() & Constants::bits_for_direct_hit) ==
                   Constants::magic_for_direct_hit;
        }
        bool is_empty() const {
            return metadata() == Constants::magic_for_empty;
        }
        bool has_next() const { return jump_index() != 0; }
        ...
        void clear_next() { set_next(0); }

        value_type& operator*() const { return block->data[index_in_block()]; }
    ...
    emplace_direct_hit(LinkedListIt block, Args&&... args) {
        using std::swap;
        if (is_full()) {
            grow();
        ...
        if (block.metadata() == Constants::magic_for_empty) {
            ...
            for (LinkedListIt it = block;;) {
                AllocatorTraits::construct(
                    *this, std::addressof(*free_block.second), std::move(*it));
            ...
            AllocatorTraits::construct(
                *this, std::addressof(*block), std::move(new_value));
            block.set_metadata(Constants::magic_for_direct_hit);
            ++num_elements;
            return { block.it(), true };
        }
    ...
    template <typename... Args>
    SKA_NOINLINE(std::pair<iterator, bool>)
    emplace_new_key(LinkedListIt parent, Args&&... args) {
        if (is_full()) {
            grow();
            return emplace(std::forward<Args>(args)...);
        }
        ...
        AllocatorTraits::construct(
            *this, std::addressof(*free_block.second),
            std::forward<Args>(args)...);
        free_block.second.set_metadata(Constants::magic_for_list_entry);
    ...
    LinkedListIt find_direct_hit(LinkedListIt child) const {
        ...
        size_t to_move_index =
            hash_policy.index_for_hash(to_move_hash, num_slots_minus_one);
    ...
    std::pair<int8_t, LinkedListIt> find_free_index(LinkedListIt parent) const {
        for (int8_t jump_index = 1; jump_index < Constants::num_jump_distances;
             ++jump_index) {
            ...
            if (block->control_bytes[index % BlockSize] ==
                Constants::magic_for_empty)
                return {
                    jump_index, { index, block }
                };
        }
        return { 0, {} };
    }

    void grow() { rehash(std::max(size_t(10), 2 * bucket_count())); }
    ...
    void deallocate_data(BlockPointer begin, size_t num_slots_minus_one) {
        ...
        unsigned char* as_byte_pointer =
            reinterpret_cast<unsigned char*>(begin);
        AllocatorTraits::deallocate(
            *this, typename AllocatorTraits::pointer(as_byte_pointer), memory);
    }
    ...
    struct convertible_to_iterator {
        ...
        operator iterator() {
            if (it->control_bytes[index % BlockSize] ==
                Constants::magic_for_empty)
...
struct CalculateBytellBlockSize<First, More...> {
    ...
    static constexpr size_t base_value =
        CalculateBytellBlockSize<More...>::value;
    static constexpr size_t value =
        this_value > base_value ? this_value : base_value;
};
template <>
struct CalculateBytellBlockSize<> {
    static constexpr size_t value = 8;
...
class bytell_hash_map :
    public detailv8::sherwood_v8_table<
        std::pair<K, V>, K, H,
        detailv8::KeyOrValueHasher<K, std::pair<K, V>, H>, E,
        detailv8::KeyOrValueEquality<K, std::pair<K, V>, E>, A,
...
public:
    ...
    friend bool
        operator==(const bytell_hash_map& lhs, const bytell_hash_map& rhs) {
...
class bytell_hash_set :
    public detailv8::sherwood_v8_table<
        T, T, H, detailv8::functor_storage<size_t, H>, E,
        detailv8::functor_storage<bool, E>, A,
        typename std::allocator_traits<A>::template rebind_alloc<unsigned char>,
...
public:
    ...
    friend bool
        operator==(const bytell_hash_set& lhs, const bytell_hash_set& rhs) {
        if (lhs.size() != rhs.size())
            return false;
