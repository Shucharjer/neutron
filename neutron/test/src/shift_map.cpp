...
void test_shift_map() {
    // default ctor
    {
        constexpr shift_map<id_t, id_t> map;

        constexpr auto result = [] {
            shift_map<id_t, id_t> map;
            return 0;
        }();
    }
...
void test_shift_map_pmr() {
    ...
    // default ctor
    { pmr::shift_map<id_t, id_t> map; }

    // alloc
    {
        { pmr::shift_map<id_t, id_t> map{ alloc }; }
        { pmr::shift_map<id_t, id_t> map{ std::allocator_arg, alloc }; }
    }
    ...
    // ctor (il)
    {
...

        {
            ...
            pmr::shift_map<id_t, id_t> map{ std::allocator_arg, alloc, ilist };
            require(map.size() == 2);
            require(map.contains(0));
            require(map.at(0) == 1);
            require(map.contains(1));
