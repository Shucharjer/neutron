...
NODISCARD inline auto find_char(
    ...
    const char* aligned = std::bit_cast<const char*>(
        (std::bit_cast<uintptr_t>(first) + alignement - 1) & mask);
...
#ifdef TARGET_X86
    #if defined(__AVX512F__) && defined(__AVX512BW__)
    for (__m512i target = _mm512_set1_epi8(character);
         iter + vector_width <= last; iter += vector_width) {
        __m512i data =
            _mm512_load_si512(reinterpret_cast<const __m512i*>(iter));
    ...
    for (__m256i target = _mm256_set1_epi8(character);
         iter + vector_width <= last; iter += vector_width) {
        __m256i data =
            _mm256_load_si256(reinterpret_cast<const __m256i*>(iter));
