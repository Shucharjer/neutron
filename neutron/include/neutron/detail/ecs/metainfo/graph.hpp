...
struct _descriptor_sys_traits {
    using prestartups = typename stage_sysinfo<pre_startup, Descriptor>::traits_list;
    using startups = typename stage_sysinfo<startup, Descriptor>::traits_list;
    using poststartups = typename stage_sysinfo<post_startup, Descriptor>::traits_list;

    using firsts = typename stage_sysinfo<first, Descriptor>::traits_list;

    using preupdates = typename stage_sysinfo<pre_update, Descriptor>::traits_list;
    using updates = typename stage_sysinfo<update, Descriptor>::traits_list;
    using postupdates = typename stage_sysinfo<post_update, Descriptor>::traits_list;
...
template <
    ...
    typename... Others>
struct _has_unresolved_predecessor<Node, Template<Others...>> : std::bool_constant<
    (false || ... || (!std::same_as<Node, Others> && _direct_before_v<Others, Node>))> {};

template <typename Node, typename Remaining>
constexpr bool _source_node_v = !_has_unresolved_predecessor<Node, Remaining>::value;
...
struct _collect_parallel_sources_impl<type_list<Selected...>, type_list<>, Remaining> {
...
struct _collect_parallel_sources_impl<
    ...
    using next = std::conditional_t<selectable, type_list<Selected..., Head>, type_list<Selected...>>;
    using type = typename _collect_parallel_sources_impl<next, type_list<Tail...>, Remaining>::type;
};

template <typename Remaining>
using _parallel_source_set_t =
    typename _collect_parallel_sources_impl<type_list<>, Remaining, Remaining>::type;
...
struct _select_current_layer {
    ...
    using type = std::conditional_t<!is_empty_template_v<parallel>, parallel, _first_source_t<Remaining>>;
};
...
struct _layer_graph<Template<Infos...>>
    : _layer_graph_impl<!is_empty_template_v<_select_current_layer_t<Template<Infos...>>>, Template<Infos...>> {};
...
template <typename Node, typename Nodes>
using _direct_predecessor_nodes_t = typename _direct_predecessor_nodes<Node, Nodes>::type;
...
template <typename Node, typename Nodes>
using _direct_successor_nodes_t = typename _direct_successor_nodes<Node, Nodes>::type;
...
struct _descendant_nodes<Node, Template<Others...>> {
    template <typename Other>
    using _predicate = std::bool_constant<
        !std::same_as<Node, Other> && _reachable<Node, Other, Template<Others...>>::value>;
...
struct stage_graph {
    ...
    static constexpr bool value = sysinfo::value && _layer_graph<traits_list>::value;
    ...
    using predecessor_indices = _node_index_list_t<
        traits_list,
        _direct_predecessor_nodes_t<node_t<Index>, traits_list>>;

    template <size_t Index>
    using successor_indices = _node_index_list_t<
        traits_list,
        _direct_successor_nodes_t<node_t<Index>, traits_list>>;

    template <size_t Index>
    using descendant_indices = _node_index_list_t<
        traits_list,
        _descendant_nodes_t<node_t<Index>, traits_list>>;
...
private:
    ...
    static consteval auto _make_index_row() noexcept {
        ...
        []<size_t... Is>(std::array<size_t, Max>& target, std::index_sequence<Is...>) {
            (void)std::initializer_list<int>{((target[Is] = value_list_element_v<Is, Values>), 0)...};
    ...
    static consteval auto _make_predecessor_counts() noexcept {
        ...
        []<size_t... Is>(std::array<size_t, size>& target, std::index_sequence<Is...>) {
            (void)std::initializer_list<int>{((target[Is] = _value_list_count<predecessor_indices<Is>>()), 0)...};
    ...
    static consteval auto _make_successor_counts() noexcept {
        std::array<size_t, size> counts{};
        []<size_t... Is>(std::array<size_t, size>& target, std::index_sequence<Is...>) {
            (void)std::initializer_list<int>{((target[Is] = _value_list_count<successor_indices<Is>>()), 0)...};
        }(counts, std::make_index_sequence<size>{});
        return counts;
    }
    ...
    static consteval auto _make_descendant_counts() noexcept {
        std::array<size_t, size> counts{};
        []<size_t... Is>(std::array<size_t, size>& target, std::index_sequence<Is...>) {
            (void)std::initializer_list<int>{((target[Is] = _value_list_count<descendant_indices<Is>>()), 0)...};
        }(counts, std::make_index_sequence<size>{});
        return counts;
    }

    static consteval size_t _max_count(const std::array<size_t, size>& counts) noexcept {
        size_t result = 0;
    ...
    static consteval auto _make_has_commands() noexcept {
        ...
        []<size_t... Is>(std::array<bool, size>& target, std::index_sequence<Is...>) {
            (void)std::initializer_list<int>{((target[Is] = node_t<Is>::has_commands), 0)...};
    ...
    static consteval auto _make_is_individual() noexcept {
        std::array<bool, size> values{};
        []<size_t... Is>(std::array<bool, size>& target, std::index_sequence<Is...>) {
            (void)std::initializer_list<int>{((target[Is] = node_t<Is>::execute_traits::is_individual), 0)...};
        }(values, std::make_index_sequence<size>{});
        return values;
    }
    ...
    static consteval auto _make_successor_matrix() noexcept {
        ...
        []<size_t... Is>(std::array<std::array<size_t, Max>, size>& target, std::index_sequence<Is...>) {
            (void)std::initializer_list<int>{((target[Is] = _make_index_row<Max, successor_indices<Is>>()), 0)...};
    ...
    static consteval auto _make_descendant_matrix() noexcept {
        std::array<std::array<size_t, Max>, size> matrix{};
        []<size_t... Is>(std::array<std::array<size_t, Max>, size>& target, std::index_sequence<Is...>) {
            (void)std::initializer_list<int>{((target[Is] = _make_index_row<Max, descendant_indices<Is>>()), 0)...};
        }(matrix, std::make_index_sequence<size>{});
        return matrix;
    }

    static consteval size_t _count_commands(const std::array<bool, size>& values) noexcept {
        size_t result = 0;
...
public:
    ...
    static constexpr size_t max_descendant_count = _max_count(descendant_counts);

    static constexpr auto has_commands = _make_has_commands();
    static constexpr auto is_individual = _make_is_individual();
    static constexpr auto successors = _make_successor_matrix<max_successor_count>();
    static constexpr auto descendants = _make_descendant_matrix<max_descendant_count>();
...
struct _descriptor_locals<Template<SysTraits...>> {
    ...
    using type = type_list_filt_t<_has_local, type_list<typename SysTraits::local...>>;
};
...
struct _descriptor_resources<Template<SysTraits...>> {
    template <typename Ty>
    using _remove_cvref = std::remove_cvref_t<Ty>;

    using type = unique_type_list_t<
        type_list_convert_t<_remove_cvref, type_list_cat_t<typename SysTraits::resource_list...>>>;
};
...
struct _descriptor_globals<Template<SysTraits...>> {
    ...
    using type = unique_type_list_t<
        type_list_convert_t<_remove_cvref, type_list_cat_t<typename SysTraits::global_list...>>>;
};
...
struct descriptor_traits {
    ...
    static constexpr bool value = graph::value;
};
