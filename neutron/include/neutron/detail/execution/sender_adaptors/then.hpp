...
struct _impls_for<then_t> : _default_impls {
    static constexpr auto complete = []<typename Ignored, typename Tag,
                                        ...
                                        typename... Args>(
                                         Ignored, State& fn, Receiver& rcvr, Tag,
                                         Args&&... args) noexcept {
        // if constexpr (std::same_as<Tag, set_value_t>) {
            ATOM_TRY {
                if constexpr (std::is_void_v<std::invoke_result_t<State&, Args...>>) {
...
// Map each child set_value_t(Args...) to set_value_t(invoke_result_t<Fn&, Args...>)
// (or set_value_t() if the result type is void). Forward set_error_t(E) and
...
struct _completion_signatures_for_impl<_basic_sender<then_t, Fn, Sndr>, Env> {
    ...
    template <typename Sig> struct _map_sig { using type = Sig; };
    ...
    template <typename... Sigs> struct _transform;
