...
struct _completion_signatures_for_impl<
    _basic_sender<just_stopped_t, _product_type<T...>>, Env> {
    using type = completion_signatures<set_stopped_t(T...)>;
};

inline constexpr just_stopped_t just_stopped;

template <>
struct _impls_for<just_stopped_t> : _default_impls {
    static constexpr auto start =
        []<typename State>(State& state, auto& rcvr) noexcept -> void {
        std::apply(
            [&rcvr](auto&... vals) mutable {
                set_stopped(std::move(rcvr), vals...);
            },
            state);
    };
};
