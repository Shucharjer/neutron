...
struct _completion_signatures_for_impl<
    _basic_sender<just_error_t, _product_type<T...>>, Env> {
    using type = completion_signatures<set_error_t(T...)>;
};

inline constexpr just_error_t just_error;

template <>
struct _impls_for<just_error_t> : _default_impls {
    static constexpr auto start =
        []<typename State>(State& state, auto& rcvr) noexcept -> void {
        std::apply(
            [&rcvr](auto&... vals) mutable {
                set_error(std::move(rcvr), vals...);
            },
            state);
    };
};
