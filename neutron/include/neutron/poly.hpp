...
concept _poly_basic_object = requires {
    ...
    requires std::is_base_of_v<
        internal::poly_empty_impl, typename Ty::template interface<internal::poly_empty_impl>>;

    // Ty should declared a alias which is a value_list called impl.
    typename Ty::template impl<typename Ty::template interface<internal::poly_empty_impl>>;
    requires is_value_list_v<
        typename Ty::template impl<typename Ty::template interface<internal::poly_empty_impl>>>;
};
...
concept _poly_has_extend = requires {
    ...
    requires std::is_base_of_v<
        internal::poly_empty_impl, typename Ty::template extends<internal::poly_empty_impl>>;
    typename Ty::template extends<internal::poly_empty_impl>::from;

    // Ty should declared a alias which is a value_list called impl.
    typename Ty::template impl<typename Ty::template extends<internal::poly_empty_impl>>;
    requires is_value_list_v<
        typename Ty::template impl<typename Ty::template extends<internal::poly_empty_impl>>>;
};
...
concept _poly_extend_object =
    _poly_has_extend<Ty> &&
    internal::poly_assert<typename Ty::template extends<internal::poly_empty_impl>::from>::value;
...
struct _vtable<Object> {
    ...
    using interface            = typename Object::template interface<internal::poly_empty_impl>;
...
class poly : private _poly_object_extract_t<Object, poly_base<poly<Object, Size, Align, Ops>>> {
    ...
    using _empty_interface = _poly_object_extract_t<Object, internal::poly_empty_impl>;
    ...
    constexpr static bool _store_vtable_as_pointer = sizeof(vtable_t) > sizeof(void*) * 2;
    ...
    /// @note The minimum size of the storage buffer is the size of 'size_type' to store the size of
    /// the object when the pointer refers to heap memory.
    alignas(Align) std::byte storage_[Size > sizeof(size_type) ? Size : sizeof(size_type)];
    ...
    constexpr void _init_ptr_from(poly&& that) {
        if (that.ptr_ == &that.storage_) {
            ...
            *static_cast<size_type*>(storage_) = *static_cast<size_type*>(std::move(that).storage_);
    ...
    constexpr static ops_t _operations() noexcept {
        ...
        if constexpr (type_list_has_v<poly_op_default_construct, ops_t>) {
            get_first<poly_op_default_construct>(ops).value =
                [](void* ptr) noexcept(std::is_nothrow_default_constructible_v<Impl>) {
                    ::new (ptr) Impl();
                };
        }
...
public:
    ...
    constexpr poly() noexcept
        : ptr_(nullptr), hash_code_(), vtable_(_vtable_value<_empty_interface>()), operations_(),
          storage_() {}

    template <_poly_impl<Object> Impl>
    constexpr poly(Impl&& impl)
        : ptr_(_init_ptr<Impl>()), hash_code_(hash_of<Impl>()), vtable_(_vtable_value<Impl>()),
          operations_(_operations<Impl>()), storage_() {
    ...
    requires type_list_has_v<poly_op_copy_construct, ops_t>
        ...
        if (that.ptr_ != nullptr) {
            _init_ptr_from(that);
            const auto operation = get_first<poly_op_copy_construct>(operations_);
    ...
    requires type_list_has_v<poly_op_move_construct, ops_t>
        : ptr_(nullptr), hash_code_(std::exchange(that.hash_code_, 0)), vtable_(that.vtable_),
          storage_(), operations_(that.operations_) {
        if (that.ptr_ != nullptr) {
            _init_ptr_from(std::move(that));
            const auto operation = get_first<poly_op_move_construct>(operations_);
            operation.value(ptr_, that.ptr_);
        }
    }
...
private:
    constexpr void _make_same(const poly& that) {
        _init_ptr_from(that);
        if constexpr (type_list_has_v<poly_op_copy_construct, ops_t>) {
            get_first<poly_op_copy_construct>(operations_).value(ptr_, that.ptr_);
        } else if constexpr (
        ...
        if (that.ptr_ != nullptr) {
            if (ptr_ != nullptr) {
                if (hash_code_ == that.hash_code_) {
                    if constexpr (type_list_has_v<poly_op_copy_assign, ops_t>) {
                        get_first<poly_op_copy_assign>(operations_).value(ptr_, that.ptr_);
                    } else if constexpr (type_list_has_v<poly_op_copy_construct, ops_t>) {
...
private:
    constexpr void _make_same(poly&& that) {
        _init_ptr_from(std::move(that));
        if constexpr (type_list_has_v<poly_op_move_construct, ops_t>) {
            get_first<poly_op_move_construct>(operations_).value(ptr_, that.ptr_);
        } else if constexpr (
            type_list_has_v<poly_op_default_construct, ops_t> &&
...
public:
        ...
        if (that.ptr_ != nullptr) {
            if (ptr_ != nullptr) {
                if (hash_code_ == that.hash_code_) {
                    if constexpr (type_list_has_v<poly_op_move_assign, ops_t>) {
                        get_first<poly_op_move_assign>(operations_).value(ptr_, that.ptr_);
                    } else if constexpr (type_list_has_v<poly_op_move_construct, ops_t>) {
                        _destroy_without_reset();
                        get_first<poly_op_move_construct>(operations_).value(ptr_, that.ptr_);
                    }
                    return *this;
    ...
    [[nodiscard]] constexpr const interface* operator->() const noexcept { return this; }
