...
void test_sparse_map();
void test_sparse_map_pmr();
...
void test_sparse_map() {
    // default ctor
    {
        constexpr sparse_map<id_t, id_t> map;

        constexpr auto result = [] {
            sparse_map<id_t, id_t> map;
            return 0;
        }();
    }
...
void test_sparse_map_pmr() {
    ...
    // default ctor
    { pmr::sparse_map<id_t, id_t> map; }

    // alloc
    {
        { pmr::sparse_map<id_t, id_t> map{ alloc }; }
        { pmr::sparse_map<id_t, id_t> map{ std::allocator_arg, alloc }; }
    }
    ...
    // ctor (il)
    {
...

        {
            ...
            pmr::sparse_map<id_t, id_t> map{ std::allocator_arg, alloc, ilist };
            require(map.size() == 2);
            require(map.contains(0));
            require(map.at(0) == 1);
            require(map.contains(1));
